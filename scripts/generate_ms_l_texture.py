...
def render_html(title: str, artist: str, font_path: str, color: tuple[int, int, int]) -> str:
...
$dst = New-Object Drawing.Bitmap 326, 50, ([Drawing.Imaging.PixelFormat]::Format32bppArgb)
...
def postprocess_no_fringe(path: Path, color: tuple[int, int, int]) -> None:
    ...
    if (width, height) != (326, 50):
        raise ValueError(f"{path}: expected 326x50, got {width}x{height}")

    for row in rows:
        for offset in range(0, len(row), 4):
            alpha = adjust_alpha(rows_alpha := row[offset + 3], 0.55, 8, 238, None)
            if alpha == 0:
                row[offset + 3] = 0
            else:
...
def row_groups(rows: list[list[int]], width: int, x0: int = 10, threshold: int = 16) -> list[tuple[int, int, int, int, int]]:
    profile = []
    for row in rows:
        count = 0
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate NOSTALGIA ms####_l title texture using the tuned Dream Han Serif JP W10 profile.")
    ...
    color_parts = tuple(int(part) for part in args.color.split(","))
    if len(color_parts) != 3 or any(part < 0 or part > 255 for part in color_parts):
        raise SystemExit("--color must be R,G,B with values 0-255")

    html_path.write_text(render_html(args.title, args.artist, args.font_path, color_parts), encoding="utf-8")
    run_edge(html_path, raw_png_win)
    resize_with_powershell(raw_png_win, args.output)
    postprocess_no_fringe(args.output, color_parts)
