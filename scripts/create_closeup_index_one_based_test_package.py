#!/usr/bin/env python3
"""Create a closeup package with note indexes shifted from 0-based to 1-based."""
...
def patch_xml(root: ET.Element) -> int:
    changed = 0
    for note in root.findall("./note_data/note"):
        index = note.find("index")
        if index is None or index.text is None:
...
def write_install(package_root: Path, mod_name: str) -> None:
    text = f"""# Closeup 1-Based Index Test Package

Variant: only `note_data/note/index` is shifted from 0-based to 1-based.
...
def create_package(base_package: Path, output_root: Path, source_mod: str, target_mod: str) -> dict[str, str | int]:
    ...
    package = output_root / "lf_closeup_index_one_based"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create closeup 1-based index test package.")
    ...
    parser.add_argument("--target-mod", default="clfn_idx1")
...
def main() -> int:
    ...
    print(
        ...
        f"files_changed={result['files_changed']} indexes_changed={result['indexes_changed']}"
    )
    return 0
